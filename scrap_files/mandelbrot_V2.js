...
const colorConfigs = {
    'dark_blue': {
        ...
        backgroundColor: '#6cf',
        baseColor: 'black',
        to_rgba:(iteration, min_iter) => [, , 17, 255 * Math.pow(0.985, iteration - min_iter)]
        }
};


const colorScheme = 'dark_blue';
const colorConfig = colorConfigs[colorScheme];
canvas.style.backgroundColor = colorConfig.backgroundColor;
document.body.style.background = colorConfig.bodyBackground;
ctx.clearRect(0, 0, canvas_width, canvas_height);
ctx.fillStyle = 'black';
...
for(let j=0; j<canvas_height; j++) {
    ...
    for(let i=0; i<canvas_width; i++) {
        ...
        if(iteration == max_iter){
            ...
            var cmap = colorConfig.to_rgba(iteration,min_iter)
        }
